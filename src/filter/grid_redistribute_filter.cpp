...
namespace xios
{
  ...
  CGridRedistributeFilter::CGridRedistributeFilter(CGarbageCollector& gc, CField* fieldIn, CField* &fieldOut) 
                                    : CFilter(gc, 1, this)
  {
    ...
    redistributeConnector_ = fieldOut->getGrid()->getRedistributeToWriterConnector(fieldIn->getGrid()) ;
  }

  CDataPacketPtr CGridRedistributeFilter::apply(std::vector<CDataPacketPtr> data)
  {
