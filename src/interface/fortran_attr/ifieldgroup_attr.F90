...
CONTAINS
  ...
  SUBROUTINE xios(set_fieldgroup_attr)  &
    ...
    IMPLICIT NONE
      ...
      REAL (KIND=8) , OPTIONAL, INTENT(IN) :: chunking_blocksize_target
  ...
  SUBROUTINE xios(set_fieldgroup_attr_hdl_)   &
    ( fieldgroup_hdl, add_offset_, axis_ref_, build_workflow_graph_, cell_methods_, cell_methods_mode_  &
    ...
    , enabled_, expr_, field_ref_, freq_offset_, freq_op_, grid_path_, grid_ref_, group_ref_, indexed_output_  &
    ...
    IMPLICIT NONE
      ...
      REAL (KIND=8) , OPTIONAL, INTENT(IN) :: chunking_blocksize_target_
      ...
      IF (PRESENT(chunking_blocksize_target_)) THEN
        CALL cxios_set_fieldgroup_chunking_blocksize_target &
      (fieldgroup_hdl%daddr, chunking_blocksize_target_)
      ENDIF
      ...
      IF (PRESENT(compression_params_)) THEN
        CALL cxios_set_fieldgroup_compression_params &
      (fieldgroup_hdl%daddr, compression_params_, SHAPE(compression_params_))
      ENDIF

      IF (PRESENT(compression_type_)) THEN
        CALL cxios_set_fieldgroup_compression_type &
      (fieldgroup_hdl%daddr, compression_type_, len(compression_type_))
      ENDIF
  ...
  SUBROUTINE xios(get_fieldgroup_attr)  &
    ...
    IMPLICIT NONE
      ...
      LOGICAL (KIND=C_BOOL) :: check_if_active_tmp
      REAL (KIND=8) , OPTIONAL, INTENT(OUT) :: chunking_blocksize_target
  ...
  SUBROUTINE xios(get_fieldgroup_attr_hdl_)   &
    ...
    IMPLICIT NONE
      ...
      LOGICAL (KIND=C_BOOL) :: check_if_active__tmp
      REAL (KIND=8) , OPTIONAL, INTENT(OUT) :: chunking_blocksize_target_
      ...
      IF (PRESENT(chunking_blocksize_target_)) THEN
        CALL cxios_get_fieldgroup_chunking_blocksize_target &
      (fieldgroup_hdl%daddr, chunking_blocksize_target_)
      ENDIF
      ...
      IF (PRESENT(compression_params_)) THEN
        CALL cxios_get_fieldgroup_compression_params &
      (fieldgroup_hdl%daddr, compression_params_, SHAPE(compression_params_))
      ENDIF

      IF (PRESENT(compression_type_)) THEN
        CALL cxios_get_fieldgroup_compression_type &
      (fieldgroup_hdl%daddr, compression_type_, len(compression_type_))
      ENDIF
  ...
  SUBROUTINE xios(is_defined_fieldgroup_attr)  &
    ...
    IMPLICIT NONE
      ...
      LOGICAL, OPTIONAL, INTENT(OUT) :: check_if_active
      LOGICAL(KIND=C_BOOL) :: check_if_active_tmp
      LOGICAL, OPTIONAL, INTENT(OUT) :: chunking_blocksize_target
      LOGICAL(KIND=C_BOOL) :: chunking_blocksize_target_tmp
      ...
      LOGICAL, OPTIONAL, INTENT(OUT) :: compression_params
  ...
  SUBROUTINE xios(is_defined_fieldgroup_attr_hdl_)   &
    ...
    IMPLICIT NONE
      ...
      LOGICAL, OPTIONAL, INTENT(OUT) :: check_if_active_
      LOGICAL(KIND=C_BOOL) :: check_if_active__tmp
      LOGICAL, OPTIONAL, INTENT(OUT) :: chunking_blocksize_target_
      LOGICAL(KIND=C_BOOL) :: chunking_blocksize_target__tmp
      ...
      LOGICAL, OPTIONAL, INTENT(OUT) :: compression_params_
      ...
      IF (PRESENT(chunking_blocksize_target_)) THEN
        chunking_blocksize_target__tmp = cxios_is_defined_fieldgroup_chunking_blocksize_target &
      (fieldgroup_hdl%daddr)
        chunking_blocksize_target_ = chunking_blocksize_target__tmp
      ENDIF
      ...
      IF (PRESENT(compression_params_)) THEN
        compression_params__tmp = cxios_is_defined_fieldgroup_compression_params &
      (fieldgroup_hdl%daddr)
        compression_params_ = compression_params__tmp
      ENDIF

      IF (PRESENT(compression_type_)) THEN
        compression_type__tmp = cxios_is_defined_fieldgroup_compression_type &
      (fieldgroup_hdl%daddr)
