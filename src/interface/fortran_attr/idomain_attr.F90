...
CONTAINS
  ...
  SUBROUTINE xios(set_domain_attr)  &
    ( domain_id, area, area_1d, area_2d, bounds_lat_1d, bounds_lat_2d, bounds_lat_name, bounds_lon_1d  &
    ...
    IMPLICIT NONE
      ...
      ( domain_hdl, area, area_1d, area_2d, bounds_lat_1d, bounds_lat_2d, bounds_lat_name, bounds_lon_1d  &
      , bounds_lon_2d, bounds_lon_name, chunking_weight_i, chunking_weight_j, comment, data_dim, data_i_index  &
      , data_ibegin, data_j_index, data_jbegin, data_ni, data_nj, dim_i_name, dim_j_name, domain_ref  &
      , i_index, ibegin, j_index, jbegin, lat_name, latvalue_1d, latvalue_2d, lon_name, long_name  &
      , lonvalue_1d, lonvalue_2d, mask_1d, mask_2d, name, ni, ni_glo, nj, nj_glo, nvertex, nvertex_name  &
  ...
  SUBROUTINE xios(set_domain_attr_hdl_)   &
    ( domain_hdl, area_, area_1d_, area_2d_, bounds_lat_1d_, bounds_lat_2d_, bounds_lat_name_, bounds_lon_1d_  &
    , bounds_lon_2d_, bounds_lon_name_, chunking_weight_i_, chunking_weight_j_, comment_, data_dim_  &
    , data_i_index_, data_ibegin_, data_j_index_, data_jbegin_, data_ni_, data_nj_, dim_i_name_  &
    , dim_j_name_, domain_ref_, i_index_, ibegin_, j_index_, jbegin_, lat_name_, latvalue_1d_, latvalue_2d_  &
    , lon_name_, long_name_, lonvalue_1d_, lonvalue_2d_, mask_1d_, mask_2d_, name_, ni_, ni_glo_  &
    , nj_, nj_glo_, nvertex_, nvertex_name_, prec_, radius_, standard_name_, type_ )
    ...
    IMPLICIT NONE
      ...
      IF (PRESENT(area_1d_)) THEN
        CALL cxios_set_domain_area_1d &
      (domain_hdl%daddr, area_1d_, SHAPE(area_1d_))
      ENDIF

      IF (PRESENT(area_2d_)) THEN
        CALL cxios_set_domain_area_2d &
      (domain_hdl%daddr, area_2d_, SHAPE(area_2d_))
      ENDIF
      ...
      IF (PRESENT(chunking_weight_i_)) THEN
        CALL cxios_set_domain_chunking_weight_i &
      (domain_hdl%daddr, chunking_weight_i_)
      ENDIF

      IF (PRESENT(chunking_weight_j_)) THEN
        CALL cxios_set_domain_chunking_weight_j &
      (domain_hdl%daddr, chunking_weight_j_)
      ENDIF
      ...
      IF (PRESENT(nvertex_name_)) THEN
        CALL cxios_set_domain_nvertex_name &
      (domain_hdl%daddr, nvertex_name_, len(nvertex_name_))
      ENDIF
  ...
  SUBROUTINE xios(get_domain_attr_hdl_)   &
    ...
    IMPLICIT NONE
      ...
      IF (PRESENT(area_1d_)) THEN
        CALL cxios_get_domain_area_1d &
      (domain_hdl%daddr, area_1d_, SHAPE(area_1d_))
      ENDIF

      IF (PRESENT(area_2d_)) THEN
        CALL cxios_get_domain_area_2d &
      (domain_hdl%daddr, area_2d_, SHAPE(area_2d_))
      ENDIF
      ...
      IF (PRESENT(chunking_weight_i_)) THEN
        CALL cxios_get_domain_chunking_weight_i &
      (domain_hdl%daddr, chunking_weight_i_)
      ENDIF

      IF (PRESENT(chunking_weight_j_)) THEN
        CALL cxios_get_domain_chunking_weight_j &
      (domain_hdl%daddr, chunking_weight_j_)
      ENDIF
      ...
      IF (PRESENT(nvertex_name_)) THEN
        CALL cxios_get_domain_nvertex_name &
      (domain_hdl%daddr, nvertex_name_, len(nvertex_name_))
      ENDIF
  ...
  SUBROUTINE xios(is_defined_domain_attr_hdl_)   &
    ...
    IMPLICIT NONE
      ...
      IF (PRESENT(area_1d_)) THEN
        area_1d__tmp = cxios_is_defined_domain_area_1d &
      (domain_hdl%daddr)
        area_1d_ = area_1d__tmp
      ENDIF

      IF (PRESENT(area_2d_)) THEN
        area_2d__tmp = cxios_is_defined_domain_area_2d &
      (domain_hdl%daddr)
      ...
      IF (PRESENT(chunking_weight_i_)) THEN
        chunking_weight_i__tmp = cxios_is_defined_domain_chunking_weight_i &
      (domain_hdl%daddr)
        chunking_weight_i_ = chunking_weight_i__tmp
      ENDIF

      IF (PRESENT(chunking_weight_j_)) THEN
        chunking_weight_j__tmp = cxios_is_defined_domain_chunking_weight_j &
      (domain_hdl%daddr)
      ...
      IF (PRESENT(nvertex_name_)) THEN
        nvertex_name__tmp = cxios_is_defined_domain_nvertex_name &
      (domain_hdl%daddr)
