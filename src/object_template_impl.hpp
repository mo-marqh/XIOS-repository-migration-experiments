...
*/
   ...
   */
   ...
   // ---------------------------------------------------------------
   template <class T>
   void CObjectTemplate<T>::setInheritedId(T* obj)
   {
     if (!hasInheritedId_)
     {
       if (!(hasId() && !hasAutoGeneratedId())) 
        if (obj->hasId() && !obj->hasAutoGeneratedId()) 
        {
          hasInheritedId_=true ;
          inheritedId_=obj->getId() ;
        } 
     }
   }

   template <class T>
   void CObjectTemplate<T>::setTemplateId(T* obj)
   {
     if (hasId() && !hasAutoGeneratedId())
     {
        templateId_ = getId() ;
     ...
     else if (obj->hasId() && !obj->hasAutoGeneratedId())
     {
       templateId_ = obj->getId() ;
       hasTemplateId_ = true ;
     }
     else if (obj->hasTemplateId())
     {
       templateId_= obj->getTemplateId() ;
       hasTemplateId_=true ;
     }
   }
