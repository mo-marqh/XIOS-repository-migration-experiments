#ifndef __LEGACY_CONTEXT_CLIENT_V2_HPP__
#define __LEGACY_CONTEXT_CLIENT_V2_HPP__
...
namespace xios
{
  ...
  CLegacyContextClientV2 processes and sends events from client to server where CContextServer receives these events
  and processes them.
  */
  class CLegacyContextClientV2 : public CContextClient
  {
    public:
      // Contructor
      CLegacyContextClientV2(CContext* parent, MPI_Comm intraComm, MPI_Comm interComm, CContext* parentServer = 0);

      // Send event to server
      ETransport getType(void) {return legacyV2 ;}
    ...
    private:
      ...
      map<int,CLegacyClientBufferV2*> buffers; //!< Buffers for connection to servers
...
#endif // __LEGACY_CONTEXT_CLIENT_V2_HPP__
