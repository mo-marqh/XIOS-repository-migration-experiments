...
namespace xios
{
  ...
  class COneSidedClientBuffer : public COneSidedCSBufferBase
  {
    class CBuffer
    {
      char* buffer_ ;
      ...
      public:
        CBuffer(MPI_Win window, size_t size, bool fixed) : start_(size), end_(0), count_(0), size_(size), fixed_(fixed), window_(window) 
        { 
        ...
        void write(char** buffer, size_t& size, MPI_Aint& addr, size_t& start, size_t& count)
        {
          ...
          if (count!=0)
          {
            ...
            {
              ERROR("COneSidedClientBuffer::CBuffer::write(char** buffer, size_t& size, MPI_Aint& addr, size_t& start, size_t& count)",<<" out of bounds"<<std::endl) ;
            }
    ...
    public:
      COneSidedClientBuffer(MPI_Comm& interComm, int serverRank, MPI_Comm& commSelf, MPI_Comm& interCommMerged, int intraServerRank) ;
