...
MODULE IFIELD
   ...
   (fieldgroup_id, name, standard_name, long_name, unit, operation, freq_op, level, &
    prec, enabled, domain_ref, axis_ref, grid_ref, field_ref, default_value)
      ...
      CALL xios(set_fieldgroup_attr_hdl_)                                                  &
         (fieldgroup_hdl, name, standard_name, long_name, unit, operation, freq_op, level, &
          prec, enabled, domain_ref, axis_ref, grid_ref, field_ref, default_value)
   ...
   (fieldgroup_hdl, name_, standard_name_, long_name_, unit_, operation_, freq_op_, level_, &
    prec_, enabled_, domain_ref_, axis_ref_, grid_ref_, field_ref_, default_value_)
      ...
      IF (PRESENT(enabled_))        THEN
         enabled__ = enabled_  
         CALL cxios_set_fieldgroup_enabled(fieldgroup_hdl%daddr, enabled__)
      END IF
   ...
   (field_id, name, standard_name, long_name, unit, operation, freq_op, level, &
    prec, enabled, domain_ref, axis_ref, grid_ref, field_ref, default_value)
      ...
      CALL xios(set_field_attr_hdl_)                                                          &
         (field_hdl, name, standard_name, long_name, unit, operation, freq_op, level,  &
          prec, enabled, domain_ref, axis_ref, grid_ref, field_ref, default_value)
   ...
   (field_hdl, name_, standard_name_, long_name_, unit_, operation_, freq_op_, level_, &
    prec_, enabled_, domain_ref_, axis_ref_, grid_ref_, field_ref_, default_value_)
      ...
      IF (PRESENT(enabled_))        THEN
         enabled__ = enabled_  
         CALL cxios_set_field_enabled(field_hdl%daddr, enabled__)
      END IF
      IF (PRESENT(domain_ref_))     THEN
