...
!!! MPI Initialization   
   ...
   ENDDO
    ...
    CALL xios_finalize()
    print*, "xios_finalize OK", rank, size

    DEALLOCATE(lon, lat, field_A, lonvalue, axisValue, field_domain)
    print*, "deallocate OK", rank, size
    ...
    print*, "server ready to end ", rank, size
  ...
  CALL MPI_COMM_RANK(MPI_COMM_WORLD, rank, ierr)
  CALL MPI_COMM_SIZE(MPI_COMM_WORLD, size, ierr)
  print*, "before MPI_Finalize : rank = ", rank, size
  CALL MPI_FINALIZE(ierr)
  !CALL VTFINI(ierr)
  print*, "after MPI_Finalize : rank = ", rank, size
