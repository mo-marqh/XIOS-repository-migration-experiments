...
def subgenerate(machine_choice):
    
    machine_name_dict={"jeanzay":"Jean-Zay",
                       "irene" : "Irene", 
                       "other": "Other"}
    
    test_list = glob.glob(machine_choice+"/test_*.txt")
    if len(test_list) == 0 :
        return
    ...
    machine = machine_choice.replace("test_","") 
    ...
    for i in range(len(myTestList)) :
        ...
        print(myTestList[i])
        file_to_open = machine_choice+"/test_"+myTestList[i][0]+"_"+myTestList[i][1]+"_"+myTestList[i][2].replace("build_","")+".txt"
    ...
    for revision in revision_list :
        algo_list = glob.glob("def_files/"+revision+"/test_*")
...
def main():
    subgenerate("test_jeanzay")
    subgenerate("test_irene")
